...
import { deleteQuestionnaireAssignmentById } from 'apiSdk/questionnaire-assignments';
...
function PatientViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PatientInterface>(
    ...
    () =>
      getPatientById(id, {
        relations: ['user', 'document', 'questionnaire_assignment'],
      }),
  );
  ...
  const questionnaire_assignmentHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('questionnaire_assignment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/questionnaire-assignments/create?patient_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.questionnaire_assignment?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/questionnaire-assignments/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess(
                                ...
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/questionnaire-assignments/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess(
                                ...
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    questionnaire_assignmentHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
